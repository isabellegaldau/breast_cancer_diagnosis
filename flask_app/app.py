...
def preprocess_image(image_bytes, image_dim):
    img = image.load_img(io.BytesIO(image_bytes), target_size=(image_dim, image_dim))
    img = image.img_to_array(img) / 255.0
    img = np.expand_dims(img, axis=0)
    return img
...
def predict():
    ...
    if file:
        try:
            # Read file into memory instead of saving it
            img_bytes = file.read()
            
            # Preprocess the image directly from memory
            img = preprocess_image(img_bytes, IMAGE_DIM)
